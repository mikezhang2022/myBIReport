(() => {
  ...
  function showGate(mode, message = '') {
    ...
    document.querySelector('#auth-title').textContent = setup ? '创建管理员账号' : '登录报表中心';
  ...
  async function submit(event) {
    ...
    try {
      const response = await originalFetch(`${apiBase}/api/auth/${setup ? 'setup' : 'login'}`, {
        ...
        body: JSON.stringify({ username: values.get('username'), displayName: values.get('username'), password })
      });
  ...
  async function start() {
    try {
      ...
      const header = document.querySelector('.topbar') || document.querySelector('main > header') || document.querySelector('header');
